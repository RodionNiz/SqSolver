...
void PrintStructPolinom (struct Polinomial);        //при необходимости выводит структуру Polinomial
void BufferClean        ();                         //очистка буффера после scanf
void menu               ();                         //меню выбора вариантов работы программы


int ParseTest            (double aRef, double bRef, double cRef, char testString []);    //один тест парсера
void RunParseTestsManual ();                                                             //запуск ручного теста парсера
